...
No current READY / approved Unity implementation packet is active. Proposed next packet: **STORY-QA-008 Strategic Map Region Playtest and Closeout Review** is READY-candidate / approval pending.

`STORY-INTEL-001`, `STORY-INTEL-002`, `STORY-INTEL-003`, `STORY-INTEL-004`, `STORY-UX-001`, `STORY-QA-005`, `STORY-CMD-001`, `STORY-CMD-002`, `STORY-CMD-003`, `STORY-CMD-004`, `STORY-QA-006`, `STORY-QA-007`, `STORY-CMD-005`, `STORY-STRAT-OBJECTIVE-001`, `STORY-TAC-AP-001`, `STORY-TAC-AI-001`, `STORY-STRAT-READ-002`, `STORY-STRAT-BASE-001`, and `STORY-STRAT-MAP-REGION-001` are DONE / merged. `STORY-QA-008` is the next guarded EPIC-007 closeout candidate and is not approved for implementation yet.

## Copy-safe prompt-file mode

If PowerShell shows `>>`, the here-string was not closed correctly. Avoid here-strings entirely and run Codex from checked-in prompt files instead.

Candidate prompt file: `production/sprints/codex-story-qa-008.prompt.txt`. It self-blocks until the story is promoted to READY / approved. Historical prompt files remain for audit only.
...
Do not run Codex until human approval promotes `STORY-QA-008` to READY / approved. The checked-in prompt file self-blocks while the story is not approved. After approval, use:
...
Do not run Codex for `STORY-QA-008` until human approval promotes the story to READY / approved. After approval, Codex must push `story/STORY-QA-008-strategic-map-region-playtest-closeout` or explicitly report why pushing/PR creation was impossible. Final response must include tests/evidence, PR URL/number if created, CI status or blocker, closeout verdict, and omissions/deferred work.
