...
async function setupSearch(
  ...
  const container = searchElement.querySelector(
    ".search-container",
  ) as HTMLElement;
  if (!container) return;
  ...
  const searchButton = searchElement.querySelector(
    ".search-button",
  ) as HTMLButtonElement;
  if (!searchButton) return;

  const searchBar = searchElement.querySelector(
    ".search-bar",
  ) as HTMLInputElement;
  if (!searchBar) return;

  const searchLayout = searchElement.querySelector(
    ".search-layout",
  ) as HTMLElement;
